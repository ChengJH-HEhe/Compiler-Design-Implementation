...
public class SemanticChecker implements astVisitor<String> {
  ...
  static typeinfo nullType = new typeinfo("null", 0, true);
  static typeinfo thisType = new typeinfo("this", 0);
  public static typeinfo[] builtinTypes = { voidType, intType, boolType, stringType, nullType, thisType };
  // builtin funcs
  ...
  public String visit(astVarDefNode node) throws error {
    // System.err.println(node.getType().getInfo().getName());
  ...
  public String visit(astCallExprNode node) throws error {
    ...
    if(funcinfo == null || !(funcinfo instanceof FuncInfo)) {
      throw new error("function " + ((astExprNode)node.getFunc()).getType().toString() + " not found");
    }
  ...
  public String visit(astArrayConstExpr node) throws error {
    ...
    if(node.getVec().size() == 0) {
      ...
      for(var expr : node.getVec()) {
        ...
        if(!((typeinfo)expr.getType()).isDim_() || ((typeinfo)expr.getType()).getDim() >((typeinfo)type).getDim()) {
          type = expr.getType();
        }
      }
