...
if [ $HAS_BUILTIN -eq 1 ]; then
    reimu -i="$TEMPDIR/test.in" -o="$TEMPDIR/test.out" -f="$TEMPDIR/builtin.s","$TEMPDIR/output.s" --all > "$TEMPDIR/reimu_output.txt"
    RAVEL_EXIT_CODE=$?
else
    reimu -i="$TEMPDIR/test.in" -o="$TEMPDIR/test.out" -f="$TEMPDIR/output.s" --all > "$TEMPDIR/reimu_output.txt"
    RAVEL_EXIT_CODE=$?
fi
